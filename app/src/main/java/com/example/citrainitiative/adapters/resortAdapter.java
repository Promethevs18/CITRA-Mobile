...
import com.example.citrainitiative.leisureactivity.Resort_Activity_Data_Displayer;
...
public class resortAdapter extends FirebaseRecyclerAdapter<display_resort_data, resortAdapter.myViewHolder> {

    Context context;
    String clickedPlace;
    DatabaseReference refresort;
    ProgressDialog pd;

    public resortAdapter(@NonNull Context context, FirebaseRecyclerOptions<display_resort_data> options) {
        super(options);
        this.context = context;
    }

    protected void onBindViewHolder(@NonNull myViewHolder holder, int position, @NonNull display_resort_data model) {
        holder.setData(model);
        display_resort_data resort = getSnapshots().get(holder.getAbsoluteAdapterPosition());
        pd = new ProgressDialog(context);

        holder.parent.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                clickedPlace = resort.getPlace();
                refresort = FirebaseDatabase.getInstance().getReference().child("Leisure").child("Resort").child(clickedPlace);
                pd.setTitle("Data Fetching");
                pd.setMessage("Data is fetching, please wait");
                pd.show();
                refresort.addValueEventListener(new ValueEventListener() {
                    @Override
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        ...
                        if (general != null) {
                            Intent a = new Intent(context, Resort_Activity_Data_Displayer.class);
    ...
    public class myViewHolder extends RecyclerView.ViewHolder {
        ...
        void setData(display_resort_data data) {
